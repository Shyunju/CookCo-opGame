...
namespace CookCo_opGame
{
    public class GameManager : Singleton<GameManager>
    {
        ...
        public List<(int, GameObject)> OrdersUI { get; set; } //주문 이미지 프리팹 리스트
        public int Score { get; private set; }
        void Awake()
        {
            _itemDataManager = new ItemDataManager();
            _recipeDataManager = new RecipeDataManager();
            OrdersUI = new List<(int, GameObject)>();
        }
        private void Start()
        {
        ...
        IEnumerator OrderNewMenuCo()
        {
            ...
            //유아이 리스트에 추가
            _orders.Add(test);
            GameObject a = Instantiate(RecipeUI[orderNumber]);
            a.transform.SetParent(_orderUICanvas.transform, true);
            a.transform.SetAsFirstSibling();
            OrdersUI.Add((orderNumber, a));  //object가 들어가는것 같지 않음 인덱스로 접근해 삭제하거나 하는 방법을 생각해야함
        }
    }
}
